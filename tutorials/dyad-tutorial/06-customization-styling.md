...
Welcome to **Chapter 6: Customization and Styling**. In this part of **Dyad Tutorial: Local-First AI App Building**, you will build an intuitive mental model first, then move into concrete implementation details and practical production tradeoffs.
...
Most teams struggle here because the hard part is not writing more code, but deciding clear boundaries for `dark`, `gray`, `theme` so behavior stays predictable as complexity grows.
...
After working through this chapter, you should be able to reason about `Chapter 6: Customization and Styling` as an operating subsystem inside **Dyad Tutorial: Local-First AI App Building**, with explicit contracts for inputs, state transitions, and outputs.

Use the implementation notes around `text`, `className`, `border` as your checklist when adapting these patterns to your own repository.

## How it Works Under the Hood

Under the hood, `Chapter 6: Customization and Styling` usually follows a repeatable control path:

1. **Context bootstrap**: initialize runtime config and prerequisites for `dark`.
2. **Input normalization**: shape incoming data so `gray` receives stable contracts.
...
- [Previous Chapter: Chapter 5: API Integration](05-api-integration.md)
- [Next Chapter: Chapter 7: Testing and Validation](07-testing-validation.md)
